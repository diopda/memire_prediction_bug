...
#
# Mode --pool (fusion projets) + split interne 80/20 (stratifié)
#   - Augmentation appliquée UNIQUEMENT sur les 80%
#   - Calibration & seuil appris sur les 20% (validation)
#   - Test sur projet(s) inédits jamais vus
#
# EXEMPLES
# 1) POOL + split 80/20 (pour simuler un gros dataset cross-proj)
# 2) SGKF (proxy LOPO rapide)
# 3) LOPO (canonique cross-project)

import argparse, json, warnings, itertools, sys, os
...
SHOW_FIGS = False  # si je veux voir les graphs ou les enregistres
...
def assemble(projects_dir: Path, names: list[str]) -> pd.DataFrame:
    ...
    for n in names:
        ...
        if f is None:
            print(f"introuvable: {n} (data_{n}.csv | {n}_matrice_final.csv | {n}.csv)")
            continue
        df = load_csv(f)
        dfs.append(df)
...
def correct_prior_shift(p, pi_train, pi_test):
    # Correction simple par décalage de logit (Saerens et al.)
    ...
    z = logit(p)
    z_adj = z + (logit(pi_test) - logit(pi_train))
...
# ======================== Configuration de Class Weights pour gerer les désequilibres ========================
    ...
    for pj in projects:
        ...
        cbs = [
            callbacks.ReduceLROnPlateau(
                monitor="val_auc_pr", mode="max", factor=0.5, patience=5, verbose=0
            ),
        ...
        if cw is not None:
            print(f"[LOPO CLASS WEIGHTS] {cw}")

        model.fit(
            pack_input(Xr_tr, Xm_tr), y_tr_oh,
        ...
        folds_info.append({
            ...
            "calib": ("temp" if T_used is not None else
                      ("isotonic" if iso_model is not None else "none")),
...
# ======================== validation  Stratified K-Fold si acitivée ========================
...
def stratk_cv_threshold_and_calib(df_trva: pd.DataFrame, METRIC_COLS, RELATION_COLS,
    ...
    for k, (idx_tr, idx_va) in enumerate(skf.split(Xm_all, y_all), start=1):
        ...
        cw = get_class_weight(y_tr, use=args.use_class_weight)
        if cw is not None:
            print(f"[StratK CLASS WEIGHTS fold={k}] {cw}")
...
# ======================== Validation StratifiedGroupKFold si activée ========================
...
def sgkf_cv_threshold_and_calib(df_trva: pd.DataFrame, METRIC_COLS, RELATION_COLS, hp, args):
    ...
    for k, (idx_tr, idx_va) in enumerate(sgkf.split(Xm_all, y_all, groups=groups), start=1):
        ...
        cw = get_class_weight(y_tr, use=args.use_class_weight)
        if cw is not None:
            print(f"[SGKF CLASS WEIGHTS fold={k}] {cw}")
...
# ======================== Helpers POOL (80/20) la methode par defaut utiliser a cause de manque de memoire ========================
...
def build_parser():
    ...
    ap.add_argument(
        "--val_freq", type=int, default=3,
        help="Keras validation_freq pour LOPO/SGKF/StratK"
    )
    ...
    # Sélection de la méthode de validation
    ap.add_argument(
        "--cv", choices=["lopo", "stratk", "sgkf", "none"], default="sgkf"
    )
    ap.add_argument(
        "--n_splits", type=int, default=3,
        help="K (SGKF/StratK). 3 = bon compromis CPU"
    )

    # Augmentation bug sur les données entrainements
    ap.add_argument(
        "--bug_aug_std", type=float, default=0.01,
        help="0 = désactivé; ex 0.02"
    )
    ap.add_argument(
        "--bug_aug_factor", type=float, default=0.5,
        help="0 = désactivé; ex 0.5 (+50%)"
    )
...
def run(args: argparse.Namespace):
    ...
    if args.pool is not None and len(args.pool) > 0:
        ...
        if args.cv != "none":
            print("[INFO] CV actif, mais en mode --pool on recommande --cv none.")
    else:
    ...
    if args.pool is None or args.cv != "none":
        if args.cv == "lopo":
            T_star, thr_star, folds_info = lopo_cv_threshold_and_calib(
                df_trva, METRIC_COLS, RELATION_COLS, hp, args,
                ppr_target_default=(args.ppr_target if args.ppr_target is not None else 0.55)
            )
            print("\n[LOPO] résumé :")
            for info in folds_info:
                print(
                    f"  - {info['project_val']}: n={info['n_val']} pi={info['pi_val']:.3f} "
            ...
            T_star, thr_star, folds_info = stratk_cv_threshold_and_calib(
                df_trva, METRIC_COLS, RELATION_COLS, hp, args
            )
            print("\n[StratK] résumé :")
            for info in folds_info:
                print(
                    f"  - fold={info['fold']}: n={info['n_val']} pi={info['pi_val']:.3f} "
                    f"calib={info['calib']} T={info['T']} thr={info['thr']:.3f} "
                    f"{info['crit']} ppr_emp={info['ppr_emp']:.3f} AUC-PR={info['auc_pr_val']:.3f}"
                )
            ...
            T_star, thr_star, folds_info = sgkf_cv_threshold_and_calib(
                df_trva, METRIC_COLS, RELATION_COLS, hp, args
            )
            print("\n[SGKF] résumé :")
            for info in folds_info:
            ...
            print("[CV] désactivé — on apprendra T/thr sur la VAL directe (si --pool) ou fallback Train+Val.")

    # 4) Préproc et entraînement final
    Xm_tr, Xr_tr, y_tr, _ = split_X_y_ids(df_tr, METRIC_COLS, RELATION_COLS)
    Xm_va, Xr_va, y_va, _ = split_X_y_ids(df_va, METRIC_COLS, RELATION_COLS)
    ...
    # 5) Probas VALIDATION (pour apprendre T et thr si CV=none)
    p_val = model.predict(pack_input(Xr_va, Xm_va), batch_size=256, verbose=0)[:, 1]
    T_used_final, iso_model_final, p_val_cal, thr_final, crit_name, ppr_emp = \
        None, None, None, None, None, None

    if T_star is None:
        # Pas de CV => calibration + thr sur VAL directe
        T_used_final, iso_model_final, p_val_cal, thr_final, crit_name, ppr_emp = \
        ...
        # Avec CV => on applique médianes T* et thr*
        thr_final = float(thr_star)
        T_used_final = float(T_star) if T_star is not None else None
        print(f"[CV->VAL] utilisation des médianes: T*={T_used_final} | thr*={thr_final:.3f}")

    # 6) Probas TEST (+ calibration*)
    ...
    # 7) Prior-shift (optionnel)
    if args.prior_shift:
        pi_train = float(np.mean(y_tr))
    ...
    if (not SHOW_FIGS) and args.out_dir:
        ...
        (out / "cv_report.json").write_text(json.dumps({
            "mode": ("pool" if (args.pool is not None and len(args.pool) > 0) else "classic"),
            "cv_mode": args.cv,
            "folds": (folds_info if len(folds_info) > 0 else None),
            "T_median": (None if T_star is None else float(T_star)),
            "thr_median": (None if thr_star is None else float(thr_star)),
            "val_T": (None if T_star is not None else
                      (None if T_used_final is None else float(T_used_final))),
            "val_thr": (None if thr_star is not None else
                        (None if thr_final is None else float(thr_final))),
